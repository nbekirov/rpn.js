define(['rpn/main'], function (rpn) {
    ...
    describe('RPN eval', function () {
        ...
        it("should NOT transform single-number non-negative expressions", function () {
            expect(rpn.evaluate('2')).toEqual(2);
            expect(rpn.evaluate('2.5')).toEqual(2.5);
        });
        ...
        it("should get the right answer for binary operators", function () {
            expect(rpn.evaluate('2 3 +')).toEqual(5);
            expect(rpn.evaluate('2 3 -')).toEqual(-1);
            expect(rpn.evaluate('2 3 *')).toEqual(6);
            expect(rpn.evaluate('2 3 x')).toEqual(6);
            expect(rpn.evaluate('2 3 /')).toBeCloseTo(0.6666);
            expect(rpn.evaluate('2 3 %')).toEqual(2);
            expect(rpn.evaluate('2 3 pow')).toEqual(8);
        });

        it("should get the right answer for unary operators", function () {
            expect(rpn.evaluate('2 _')).toEqual(-2);
        });
    });
});